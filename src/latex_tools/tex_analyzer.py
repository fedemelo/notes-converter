...
async def replace_command_with_delimiters(
    file: UploadFile, command: str, left_delimiter: str, right_delimiter: str
) -> str:
...
def replace_command_with_delimiters_by_counting_brackets(
    tex_content: str, command: str, left_delimiter: str, right_delimiter: str
) -> str:
    """
    This function replaces each instance of a command with delimiters.

    It uses a (theoretical) stack (a counter) to keep track of the number of opening
    and closing brackets. When the stack is empty, the function knows that it has found
    the closing bracket of the command.

    This replacement can't be done with a simple regex because the command's argument
        ...
        if is_stacking:
            if tex_content[i] == "{":
                ...
                if stack == 1:
                    ...
                    i += 1
                    continue  # Don't add the opening bracket to the new content
            elif tex_content[i] == "}":
                stack -= 1
                ...
                if stack == 0:
                    ...
                    i += 1
                    continue  # Don't add the closing bracket to the new content
